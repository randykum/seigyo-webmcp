...
export function registerSeigyoTools(): () => void {
  ...
  const tools: Tool[] = [
    ...
    { name: "seigyo.verify_action", description: "Verify the observed recovery result and create a tamper-evident receipt.", inputSchema: { type: "object", properties: { executionId: { type: "string" }, incidentId: { type: "string" } }, required: ["executionId", "incidentId"], additionalProperties: false }, annotations: { readOnlyHint: true }, execute: async input => { const value = parseToolInput(input, z.object({ executionId: z.string(), incidentId: z.string() })); return textResult(await api.verify(value.executionId, value.incidentId)); } },

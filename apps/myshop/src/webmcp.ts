...
export function registerMyShopTools(): () => void {
  ...
  const tools: Tool[] = [
    ...
    { name: "myshop.checkout", description: "Complete a simulated checkout. No real payment is charged. Customer details are used only inside the deterministic simulation.", inputSchema: { type: "object", properties: { email: { type: "string" }, name: { type: "string" }, address: { type: "string" }, cartId: { type: "string" }, requestId: { type: "string" }, idempotencyKey: { type: "string" } }, required: ["email", "name", "address", "requestId", "idempotencyKey"], additionalProperties: false }, execute: async input => { const value = parseToolInput(input, z.object({ email: z.string().email(), name: z.string().min(2), address: z.string().min(8), cartId: z.string().default(CART_ID), requestId: z.string().min(8), idempotencyKey: z.string().min(8) })); return result(await shopApi.checkout(value)); } },
    { name: "myshop.get_order", description: "Look up one simulated order by order ID.", inputSchema: { type: "object", properties: { orderId: { type: "string" } }, required: ["orderId"], additionalProperties: false }, annotations: { readOnlyHint: true }, execute: async input => { const value = parseToolInput(input, z.object({ orderId: z.string() })); return result(await shopApi.order(value.orderId)); } },

...
const base = {
  "storefront-edge": { demand: 100, capacity: 800, p95: 120, error: 0.002 },
  "catalog-api": { demand: 45, capacity: 160, p95: 180, error: 0.003 },
  "cart-api": { demand: 12, capacity: 80, p95: 220, error: 0.004 },
  "checkout-api": { demand: 10, capacity: 60, p95: 420, error: 0.006 },
  "payment-gateway": { demand: 9, capacity: 45, p95: 480, error: 0.008 },
  "inventory-db": { demand: 130, capacity: 100, p95: 90, error: 0.002 },
  "order-worker": { demand: 8, capacity: 25, p95: 700, error: 0.005 }
} satisfies Record<ServiceId, { demand: number; capacity: number; p95: number; error: number }>;

const dependencies: Record<ServiceId, Array<{ id: ServiceId; weight: number }>> = {
  "storefront-edge": [{ id: "catalog-api", weight: 0.24 }, { id: "cart-api", weight: 0.12 }, { id: "checkout-api", weight: 0.1 }],
  "catalog-api": [{ id: "inventory-db", weight: 0.35 }],
  "cart-api": [{ id: "inventory-db", weight: 0.28 }],
  "checkout-api": [{ id: "inventory-db", weight: 0.2 }, { id: "payment-gateway", weight: 0.65 }, { id: "order-worker", weight: 0.08 }],
  "payment-gateway": [],
  "inventory-db": [],
  "order-worker": [{ id: "inventory-db", weight: 0.12 }]
};
...
const productSpecs: Array<[string, Product["category"], string, number, string]> = [
  ["Kuro lounge chair", "Furniture", "Blackened oak and cane", 1180, "oak-chair.png"], ["Fold paper pendant", "Lighting", "Handmade washi paper", 620, "paper-pendant.png"], ["Umber vessel", "Ceramics", "Mineral clay", 185, "umber-vessel.png"], ["Axis side table", "Furniture", "Honed travertine", 790, "hero-living-room.png"],
  ["Lowline bench", "Furniture", "Smoked oak", 940, "oak-chair.png"], ["Halo floor lamp", "Lighting", "Paper and blackened steel", 860, "paper-pendant.png"], ["Still bowl", "Ceramics", "Speckled stoneware", 140, "umber-vessel.png"], ["Grid wool throw", "Textiles", "Brushed merino wool", 260, "hero-living-room.png"],
  ["Arc dining chair", "Furniture", "Oak and woven rush", 540, "oak-chair.png"], ["Orbit pendant", "Lighting", "Layered paper", 710, "paper-pendant.png"], ["Quiet carafe", "Ceramics", "Glazed stoneware", 165, "umber-vessel.png"], ["Linen field cushion", "Textiles", "Washed Belgian linen", 120, "hero-living-room.png"],
  ["Monolith stool", "Furniture", "Solid ash", 420, "oak-chair.png"], ["Veil table lamp", "Lighting", "Paper and oak", 390, "paper-pendant.png"], ["Earth cup set", "Ceramics", "Reduction-fired clay", 96, "umber-vessel.png"], ["Raw edge runner", "Textiles", "Handwoven flax", 150, "hero-living-room.png"],
  ["Plane coffee table", "Furniture", "Black steel and stone", 890, "oak-chair.png"], ["Moon wall light", "Lighting", "Plaster and brass", 470, "paper-pendant.png"], ["Dune platter", "Ceramics", "Sand stoneware", 210, "umber-vessel.png"], ["Soft grid blanket", "Textiles", "Wool and alpaca", 320, "hero-living-room.png"],
  ["Cut bookend pair", "Objects", "Honed limestone", 230, "umber-vessel.png"], ["Long tray", "Objects", "Blackened oak", 170, "oak-chair.png"], ["Paper study mobile", "Objects", "Paper and fine steel", 280, "paper-pendant.png"], ["Quiet hour candle", "Objects", "Beeswax and ceramic", 78, "hero-living-room.png"]
];

const createProducts = (): Product[] => productSpecs.map((p, index) => ({ id: id("PRD", index + 1), slug: p[0].toLowerCase().replaceAll(" ", "-"), name: p[0], category: p[1], material: p[2], description: `A considered ${p[0].toLowerCase()} designed around honest material, useful proportion, and a quiet presence in the room.`, price: p[3], inventory: 5 + (index * 7) % 19, image: `/images/${p[4]}`, dimensions: index % 2 === 0 ? "W 68 x D 74 x H 71 cm" : "W 42 x D 42 x H 54 cm", featured: index < 8 }));

const scenarioInfo: Record<ScenarioId, { title: string; incidentId: string; serviceId: ServiceId; impact: string; cause: string; severity: Incident["severity"] }> = {
  "checkout-regression": { title: "Checkout errors after deployment", incidentId: "INC-042", serviceId: "checkout-api", impact: "Customers receive validation failures during checkout.", cause: "checkout-2026.08.30.4 with new-tax-rounding enabled", severity: "critical" },
  "payment-outage": { title: "Primary payment provider unavailable", incidentId: "INC-043", serviceId: "payment-gateway", impact: "Payment authorizations are timing out across checkout.", cause: "External primary provider outage", severity: "critical" },
  "inventory-saturation": { title: "Inventory database saturation", incidentId: "INC-044", serviceId: "inventory-db", impact: "Product availability and reservations are delayed.", cause: "Inventory demand exceeds provisioned capacity", severity: "high" }
};

const createIncidents = (scenario: ScenarioId, now: number): Incident[] => {
  const active = scenarioInfo[scenario];
  const history = Array.from({ length: 29 }, (_, index): Incident => ({ id: id("INC", index + 1), title: ["Elevated catalog latency", "Order worker retry spike", "Cart cache churn", "Edge origin timeout"][index % 4] ?? "Historical incident", severity: (["low", "medium", "high"] as const)[index % 3] ?? "low", status: "resolved", serviceId: (Object.keys(base) as ServiceId[])[index % 7] ?? "storefront-edge", startedAt: now - (index + 3) * 86_400_000, updatedAt: now - (index + 3) * 86_400_000 + 2_400_000, resolvedAt: now - (index + 3) * 86_400_000 + 2_400_000, impact: "A bounded production degradation affected a subset of customer requests.", customerErrorsPerMinute: 12 + index, ordersAtRisk: 2 + index % 8, cause: "Resolved historical operating condition" }));
  return [{ id: active.incidentId, title: active.title, severity: active.severity, status: "investigating", serviceId: active.serviceId, startedAt: now - 21 * 60_000, updatedAt: now, impact: active.impact, customerErrorsPerMinute: scenario === "payment-outage" ? 542 : scenario === "checkout-regression" ? 186 : 94, ordersAtRisk: scenario === "payment-outage" ? 88 : scenario === "checkout-regression" ? 42 : 27, cause: active.cause }, ...history];
};

const createDeployments = (now: number): Deployment[] => Array.from({ length: 60 }, (_, index) => {
  const serviceId = (Object.keys(base) as ServiceId[])[index % 7] ?? "storefront-edge";
...
export const seedEnvironment = (scenario: ScenarioId = "checkout-regression", epoch = 1): EnvironmentState => {
  ...
  for (let tick = 720; tick > 0; tick -= 1) {
    ...
    state.metrics.push(...health.map(point => ({ ...point, timestamp, p50Ms: round(point.p95Ms * 0.43, 1), cpuPct: round(clamp(point.utilization * 62 + hash01(`${state.seed}:${timestamp}:${point.serviceId}`) * 8, 2, 99), 1) })));
  }
  ...
  for (let index = 0; index < 1500; index += 1) {
    ...
    state.logs.push({ id: id("LOG", index + 1), timestamp, serviceId, level, eventName: level === "error" ? "request.failed" : level === "warn" ? "latency.threshold" : "request.completed", message: level === "error" ? `${serviceId} request failed after dependency response` : level === "warn" ? `${serviceId} p95 latency crossed the warning threshold` : `${serviceId} completed request`, traceId: `tr_${shortHash(`${index}:trace`)}`, requestId: `req_${shortHash(`${index}:request`)}`, metadata: { durationMs: 42 + (index * 17) % 1800, attempt: 1 + index % 3 } });
  }
  return state;
};
...
const directHealth = (state: EnvironmentState, serviceId: ServiceId, timestamp: number): ServiceHealth => {
  ...
  const providerCapacityFactor = serviceId === "payment-gateway" && service.provider === "fallback" ? 0.7 : 1;
  const capacity = config.capacity * service.replicas * providerCapacityFactor;
  const utilization = demand / capacity;
  let faultError = 0;
  let faultLatency = 0;
  if (serviceId === "checkout-api" && service.version === "checkout-2026.08.30.4" && service.featureFlags["new-tax-rounding"]) { faultError = 0.17; faultLatency = 700; }
  if (serviceId === "payment-gateway" && service.provider === "primary" && !state.primaryProviderAvailable) { faultError = 0.982; faultLatency = 3000; }
  if (serviceId === "payment-gateway" && service.provider === "fallback") { faultError = 0.012; faultLatency = 30; }
  ...
  const noise = (hash01(`${state.seed}:${state.epoch}:${timestamp}:${serviceId}`) - 0.5) * 0.012;
  const errorRate = clamp(config.error + saturationPenalty + faultError + restartError + maintenanceError + noise, 0, 0.99);
  const p95Ms = Math.min(30000, config.p95 + queueMs + faultLatency + Math.abs(noise) * 1000);
...
export const computeHealth = (state: EnvironmentState, timestamp = state.virtualNow): ServiceHealth[] => {
  ...
  for (const serviceId of order) {
    ...
    const dependencyError = dependencies[serviceId].reduce((sum, dependency) => sum + (map.get(dependency.id)?.errorRate ?? 0) * dependency.weight, 0);
    const dependencyLatency = dependencies[serviceId].reduce((sum, dependency) => sum + (map.get(dependency.id)?.p95Ms ?? 0) * dependency.weight, 0);
    const errorRate = clamp(own.errorRate + dependencyError, 0, 0.99);
    const p95Ms = Math.min(30000, own.p95Ms + dependencyLatency);
    map.set(serviceId, { ...own, errorRate: round(errorRate, 4), availability: round(1 - errorRate, 4), p95Ms: round(p95Ms, 1), status: own.status === "maintenance" ? "maintenance" : errorRate > 0.2 ? "critical" : errorRate > 0.02 || p95Ms > base[serviceId].p95 * 2 ? "degraded" : "healthy" });
  }
  return (Object.keys(base) as ServiceId[]).map(serviceId => map.get(serviceId) as ServiceHealth);
};
...
export const tick = (state: EnvironmentState, seconds = 30): EnvironmentState => {
  ...
  state.metrics.push(...health.map(point => ({ ...point, timestamp: state.virtualNow, p50Ms: round(point.p95Ms * 0.43, 1), cpuPct: round(clamp(point.utilization * 62 + hash01(`${state.seed}:${state.virtualNow}:${point.serviceId}`) * 8, 2, 99), 1) })));
  if (state.metrics.length > 6000) state.metrics.splice(0, state.metrics.length - 6000);
  const worst = [...health].sort((a, b) => b.errorRate - a.errorRate)[0];
  if (worst) state.logs.push({ id: id("LOG", state.logs.length + 1), timestamp: state.virtualNow, serviceId: worst.serviceId, level: worst.errorRate > 0.1 ? "error" : worst.errorRate > 0.02 ? "warn" : "info", eventName: worst.errorRate > 0.1 ? "request.failed" : "health.sample", message: worst.errorRate > 0.1 ? `${worst.serviceId} observed elevated request failures` : `${worst.serviceId} health sample recorded`, traceId: `tr_${shortHash(`${state.virtualNow}:trace`)}`, metadata: { errorRate: worst.errorRate, p95Ms: worst.p95Ms } });
...
const isCorrectAction = (state: EnvironmentState, action: RecoveryAction): "recovery" | "partial" | "containment" | "unlikely" => {
  ...
  if (state.scenario === "checkout-regression") {
    ...
    if (action.targetService === "checkout-api" && action.type === "disable_feature") return "partial";
  }
  if (state.scenario === "payment-outage" && action.targetService === "payment-gateway" && action.type === "shift_traffic") return "recovery";
  if (state.scenario === "payment-outage" && action.targetService === "payment-gateway" && action.type === "switch_provider" && action.parameters.provider === "fallback") return "recovery";
  if (state.scenario === "inventory-saturation" && action.targetService === "inventory-db" && action.type === "scale_service") return Number(action.parameters.replicas ?? 0) >= 4 ? "recovery" : "partial";
  return "unlikely";
};
...
const validateAction = (state: EnvironmentState, action: RecoveryAction): string | null => {
  ...
  if (action.type === "disable_feature" && (typeof action.parameters.feature !== "string" || !action.parameters.feature.trim() || !(action.parameters.feature in service.featureFlags))) return "A declared feature on the target service is required.";
  if (action.type === "switch_provider" && (action.targetService !== "payment-gateway" || !["primary", "fallback"].includes(String(action.parameters.provider)))) return "Provider switching requires payment-gateway and provider primary or fallback.";
  if (action.type === "shift_traffic" && !["checkout-api", "payment-gateway"].includes(action.targetService)) return "Traffic shifting is supported only for checkout-api or payment-gateway.";
  if (action.type === "rollback_deployment" && !state.deployments.some(item => item.serviceId === action.targetService && item.version === service.version && item.status === "success")) return "No current successful deployment can be rolled back on the target service.";
  return null;
};
...
const applyAction = (state: EnvironmentState, action: RecoveryAction): void => {
  ...
  if (action.type === "scale_service") service.replicas = action.parameters.replicas as number;
  if (action.type === "shift_traffic" && service.id === "checkout-api") { const deployment = state.deployments.find(item => item.serviceId === service.id && item.version === service.version); service.version = deployment?.previousVersion ?? service.previousVersion; if (deployment) deployment.status = "rolled_back"; service.featureFlags["new-tax-rounding"] = false; }
  if (action.type === "shift_traffic" && service.id === "payment-gateway") service.provider = "fallback";
  if (action.type === "disable_feature") service.featureFlags[String(action.parameters.feature)] = false;
  if (action.type === "switch_provider") service.provider = action.parameters.provider === "primary" ? "primary" : "fallback";
...
export const investigate = (state: EnvironmentState, incidentId: string) => {
  ...
  const deployment = state.deployments.find(item => item.serviceId === root.serviceId);
  const result = { incident, summary: `${root.serviceId} is the highest-confidence causal source for the active customer impact.`, hypotheses: [{ id: "H1", statement: root.cause, confidence: 0.92, evidenceRefs: [`metric:${root.serviceId}`, deployment ? `deployment:${deployment.id}` : `logs:${root.serviceId}`] }, { id: "H2", statement: "A downstream dependency is amplifying retry pressure.", confidence: 0.48, evidenceRefs: ["dependency:checkout-path"] }], signals: health.filter(item => item.status !== "healthy").map(item => ({ kind: "metric", sourceRef: `metric:${item.serviceId}`, observedAt: state.virtualNow, detail: `${item.serviceId} error ${round(item.errorRate * 100, 2)}%, p95 ${item.p95Ms} ms` })), nextBestActions: state.scenario === "checkout-regression" ? [{ actionType: "rollback_deployment", targetService: "checkout-api", reason: "The error increase follows the current checkout deployment." }] : state.scenario === "payment-outage" ? [{ actionType: "switch_provider", targetService: "payment-gateway", reason: "The primary provider is unavailable while the fallback remains healthy." }] : [{ actionType: "scale_service", targetService: "inventory-db", reason: "Demand exceeds provisioned database capacity." }], staleAt: state.virtualNow + 5 * 60_000 };
  recordActivity(state, "seigyo.investigate_incident", "Correlate metrics, logs, deployments, and dependencies", `Identified ${root.serviceId} as the leading cause.`);
  return result;
};
...
export const proposeAction = async (state: EnvironmentState, input: ProposalInput): Promise<Proposal> => {
  ...
  if (duplicate) {
    if (duplicate.fingerprint !== fingerprint) throw new Error("IDEMPOTENCY_CONFLICT");
  ...
  if (incident.status === "resolved") throw new Error("PRECONDITION_FAILED:Resolved incidents cannot accept new interventions.");
  if (input.evidenceRefs.length < 1) throw new Error("PRECONDITION_FAILED:At least one evidence reference is required.");
  const evidenceIsValid = (reference: string): boolean => {
    const [kind, value] = reference.split(":", 2);
    if (!kind || !value) return false;
    if (kind === "deployment") return state.deployments.some(item => item.id === value);
    if (kind === "metric" || kind === "logs") return value in state.services;
    if (kind === "dependency") return value === "checkout-path" || value in state.services;
    return false;
  };
  if (!input.evidenceRefs.every(evidenceIsValid)) throw new Error("INVALID_ARGUMENT:One or more evidence references are invalid.");
  ...
  const current = computeHealth(state).find(item => item.serviceId === input.action.targetService) ?? computeHealth(state)[0] as ServiceHealth;
  const projected = structuredClone(state);
  applyAction(projected, input.action);
  const projectedHealth = computeHealth(projected).find(item => item.serviceId === input.action.targetService) ?? current;
  const proposal: Proposal = { id: id("PROP", state.proposals.length + 1), incidentId: input.incidentId, epoch: state.epoch, causalRevision: state.causalRevision, action: input.action, actionHash: await actionFingerprint(input.action), rationale: input.rationale, evidenceRefs: input.evidenceRefs, status: "pending", expiresAt: state.virtualNow + 10 * 60_000, predictedImpact: { classification, risk: input.action.type === "maintenance_mode" ? "high" : input.action.type === "restart_service" ? "medium" : "low", summary: classification === "recovery" ? "Expected to remove the modeled root condition." : classification === "partial" ? "Expected to improve the incident without fully removing the cause." : classification === "containment" ? "Stops customer traffic but does not restore service." : "Changes service state but is unlikely to remove the current cause.", expectedErrorRate: round(projectedHealth.errorRate, 4), expectedP95Ms: projectedHealth.p95Ms, reversible: input.action.type !== "maintenance_mode" } };
  state.proposals.push(proposal);
  state.idempotency[input.idempotencyKey] = { fingerprint, result: proposal };
  recordActivity(state, "seigyo.propose_action", "Create a reviewable intervention", `${input.action.type} proposed for ${input.action.targetService}.`);
  return proposal;
};
...
export const approveProposal = async (state: EnvironmentState, proposalId: string, sessionId: string): Promise<Approval> => {
  ...
  if (proposal.expiresAt <= state.virtualNow) { proposal.status = "expired"; throw new Error("STALE_STATE"); }
  if (proposal.epoch !== state.epoch || proposal.causalRevision !== state.causalRevision) { proposal.status = "stale"; throw new Error("STALE_STATE"); }
  const approval: Approval = { proposalId, sessionId, actionHash: proposal.actionHash, epoch: state.epoch, causalRevision: state.causalRevision, token: `${crypto.randomUUID()}${crypto.randomUUID()}`.replaceAll("-", ""), expiresAt: state.virtualNow + 5 * 60_000, used: false, approvedAt: state.virtualNow };
...
export const executeProposal = async (state: EnvironmentState, proposalId: string, approvalToken: string, idempotencyKey: string, sessionId: string): Promise<Execution> => {
  ...
  const proposal = state.proposals.find(item => item.id === proposalId);
  const approval = state.approvals.find(item => item.proposalId === proposalId && item.token === approvalToken);
  if (!proposal) throw new Error("NOT_FOUND");
  if (proposal.status !== "approved") throw new Error("PRECONDITION_FAILED:Proposal is not approved for execution.");
...
export const verifyExecution = async (state: EnvironmentState, executionId: string, incidentId: string): Promise<Verification> => {
  ...
  tick(state, 30); tick(state, 30); tick(state, 30);
  const health = computeHealth(state);
  const inventory = health.find(item => item.serviceId === "inventory-db") as ServiceHealth;
  const target = health.find(item => item.serviceId === execution.action.targetService) as ServiceHealth;
  const correct = isCorrectAction(state, execution.action);
  const errorThreshold = execution.action.targetService === "checkout-api" ? 0.05 : execution.action.targetService === "payment-gateway" ? 0.03 : 0.02;
...
export const undoExecution = async (state: EnvironmentState, executionId: string, idempotencyKey: string): Promise<Execution> => {
  ...
  service.version = saved.version; service.previousVersion = saved.previousVersion; service.replicas = saved.replicas; service.enabled = saved.enabled; service.restartUntil = saved.restartUntil; service.featureFlags = { ...saved.featureFlags }; if (saved.provider) service.provider = saved.provider;
...
export const updateCart = (state: EnvironmentState, cartId: string, productId: string, quantity: number): Cart => {
  ...
  if (existing) existing.quantity = quantity; else if (quantity > 0) cart.items.push({ productId, quantity });
...
export const checkout = async (state: EnvironmentState, input: { cartId: string; email: string; name: string; address: string; requestId: string; idempotencyKey: string }): Promise<Order> => {
  ...
  const fingerprint = await sha256(canonical(input));
  const duplicate = Object.hasOwn(state.idempotency, input.idempotencyKey) ? state.idempotency[input.idempotencyKey] : undefined;
  if (duplicate) { if (duplicate.fingerprint !== fingerprint) throw new Error("IDEMPOTENCY_CONFLICT"); return duplicate.result as Order; }
  const cart = getCart(state, input.cartId);
  if (cart.items.length === 0) throw new Error("PRECONDITION_FAILED:Cart is empty.");
  const checkoutService = state.services["checkout-api"];
  if (!checkoutService.enabled) throw new Error("UPSTREAM_UNAVAILABLE:Checkout is temporarily in maintenance mode.");
  if (!["payment-gateway", "inventory-db", "order-worker"].every(serviceId => state.services[serviceId as ServiceId].enabled)) throw new Error("UPSTREAM_UNAVAILABLE:A checkout dependency is temporarily unavailable. Your cart is safe.");
  if (state.scenario === "checkout-regression" && checkoutService.version === "checkout-2026.08.30.4" && checkoutService.featureFlags["new-tax-rounding"] && hash01(`${state.seed}:${input.requestId}`) < 0.4) throw new Error("UPSTREAM_UNAVAILABLE:We could not validate this checkout. Your cart is safe. Please try again.");
  if (state.scenario === "payment-outage" && state.services["payment-gateway"].provider === "primary" && !state.primaryProviderAvailable) throw new Error("UPSTREAM_UNAVAILABLE:Payment authorization is taking longer than expected. No charge was made.");
  if (state.scenario === "inventory-saturation" && state.services["inventory-db"].replicas < 4 && hash01(`${state.seed}:${input.requestId}:inventory`) < 0.7) throw new Error("UPSTREAM_UNAVAILABLE:We could not confirm inventory in time. Your cart is unchanged.");
  const items = cart.items.map(item => { const product = state.products.find(entry => entry.id === item.productId); if (!product || product.inventory < item.quantity) throw new Error("PRECONDITION_FAILED:An item is no longer available in the requested quantity."); return { productId: item.productId, quantity: item.quantity, unitPrice: product.price }; });
  const subtotal = items.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0);
  const order: Order = { id: id("ORD", state.orders.length + 1001), cartId: cart.id, email: input.email, name: input.name, address: input.address, total: subtotal + (subtotal >= 500 ? 0 : 45), status: "confirmed", createdAt: state.virtualNow, items };
  for (const item of items) { const product = state.products.find(entry => entry.id === item.productId); if (product) product.inventory -= item.quantity; }
...
export const createTraceId = (): string => `tr_${crypto.randomUUID().replaceAll("-", "").slice(0, 16)}`;

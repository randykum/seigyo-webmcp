...
export type ApiResult<T> =
  | { ok: true; data: T; stateVersion: number; traceId: string }
  | {
      ok: false;
      error: {
        code: ErrorCode;
...
export interface OperationalStatus {
  ...
  customerErrorsPerMinute: number;
  ordersAtRisk: number;
  updatedAt: number;
  incidentId?: string;
}
export interface MetricPoint extends ServiceHealth {
  timestamp: number;
  p50Ms: number;
  cpuPct: number;
}
...
export interface Proposal {
  ...
  expiresAt: number;
  predictedImpact: {
    classification: "recovery" | "partial" | "containment" | "unlikely";
...
export interface Receipt {
  ...
  executedAt: number;
  verifiedAt?: number;
  result: VerificationOutcome | "pending";
...
export interface Product {
  ...
  slug: string;
  name: string;
  category: "Furniture" | "Lighting" | "Ceramics" | "Textiles" | "Objects";
  material: string;
  description: string;
  price: number;
  inventory: number;
